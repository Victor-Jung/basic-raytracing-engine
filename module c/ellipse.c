...
void* doesCollideEllipse (sEllipse ellipse, sParamEqua lightRay){
	...
	G = 2*pow(ellipse.beta*ellipse.gamma,2)*lightRay.x[0]*lightRay.x[1] - 2*pow(ellipse.beta*ellipse.gamma,2)*lightRay.x[0]*ellipse.a + 2*pow(ellipse.alpha*ellipse.gamma,2)*lightRay.y[0]*lightRay.y[1] - 2*pow(ellipse.alpha*ellipse.gamma,2)*lightRay.y[0]*ellipse.b + 2*pow(ellipse.beta*ellipse.alpha,2)*lightRay.z[0]*lightRay.z[1] - 2*pow(ellipse.beta*ellipse.alpha,2)*lightRay.z[0]*ellipse.b;
	H = pow(ellipse.beta * ellipse.gamma * ellipse.a,2) + pow(ellipse.beta * ellipse.gamma * lightRay.x[1],2) - 2*pow(ellipse.beta * ellipse.gamma,2)*ellipse.a*lightRay.x + pow(ellipse.alpha * ellipse.gamma * ellipse.b,2) + pow(ellipse.alpha * ellipse.gamma * lightRay.y[1],2) - 2*pow(ellipse.alpha * ellipse.gamma,2)*ellipse.b*lightRay.y + pow(ellipse.beta * ellipse.alpha * ellipse.b,2) + pow(ellipse.beta * ellipse.alpha * lightRay.y[1],2) - 2*pow(ellipse.beta * ellipse.alpha,2)*ellipse.b*lightRay.y - pow(ellipse.alpha*ellipse.beta*ellipse.gamma,2);
